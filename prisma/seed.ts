...
function referenceUrl(name: string) {
  return `https://www.google.com/search?q=${encodeURIComponent(`site:exrx.net ${name}`)}`;
}
...
const METADATA: Record<string, MetadataSeed> = {
  ...
  e18: { kind: 'isolation', primaryMuscles: ['biceps', 'forearms'], secondaryMuscles: [], stabilizerMuscles: ['core'], instructions: ['Mantene mueca neutra y codo cerca del cuerpo desde la primera rep.', 'Flexiona pensando en llevar el pulgar hacia el hombro sin balancear.', 'Baja con control para no perder tension del braquial y antebrazo.'], commonMistakes: ['Girar demasiado la mueca y perder el foco martillo.', 'Balancear el torso para iniciar el movimiento.', 'Acortar la excentrica por fatiga.'], technicalCues: ['Mueca neutra.', 'Sin balanceo.', 'Bajada completa.'], overloadRecommendation: 'Subi peso solo si no aparece balanceo y mantenes mueca neutra.' },
