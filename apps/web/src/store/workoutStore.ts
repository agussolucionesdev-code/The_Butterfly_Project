...
interface WorkoutState {
  ...
  timerSeconds: number; timerActive: boolean; timerPaused: boolean; completed: boolean;
  setWorkout: (cycleDay: number, cycleDate: string, day: TrainingDay, logs: SetLog[]) => void; addLog: (log: SetLog) => void;
  startTimer: (seconds: number) => void; tickTimer: () => void; addTimerSeconds: (seconds: number) => void; togglePause: () => void; skipTimer: () => void; advance: () => void;
...
export const useWorkoutStore = create<WorkoutState>((set, get) => ({
  cycleDay: 1, cycleDate: '', day: null, logs: [], exerciseIndex: 0, setNumber: 1, timerSeconds: 0, timerActive: false, timerPaused: false, completed: false,
  setWorkout: (cycleDay, cycleDate, day, logs) => set({ cycleDay, cycleDate, day, logs, ...derivePosition(day, logs), timerSeconds: 0, timerActive: false, timerPaused: false }),
  addLog: (log) => set((state) => ({ logs: [...state.logs.filter((item) => item.id !== log.id), log] })),
  startTimer: (seconds) => set({ timerSeconds: seconds, timerActive: seconds > 0, timerPaused: false }),
  ...
  addTimerSeconds: (seconds) => set((state) => ({ timerSeconds: state.timerSeconds + seconds })),
